...
from fastui.components import Page, FireEvent
...
def error_page() -> list[AnyComponent]:
    return [Page(components=[
        ...
        c.error_section(text="An error occured on the server. We are trying to fix it as soon as possible", width=8)
    ])]
...
async def post_paste(form: Annotated[NewPasteForm, fastui_form(NewPasteForm)]):   
    async with ClientSession(BACKEND_BASE_URL) as session:
        async with session.post("/api/paste/", json=form.model_dump(exclude_none=True)) as resp:
            if resp.status == 201:
